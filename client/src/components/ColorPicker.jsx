...
const ColorPicker = ({selectedColor, onChange}) => {
  ...
  return (
    <div className='relative'>
        ...
        {isOPen && (
            <div className='grid grid-cols-4 top-full w-60 gap-2 absolute left-0 right-0 p-3 mt-2 bg-white z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {colors.map((color)=>(
                    <div key={color.value} onClick={()=>{onChange(color.value);setIsOpen(false)}} className='realtive group flex flex-col cursor-pointer'>
