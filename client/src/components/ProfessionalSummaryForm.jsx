...
const ProfessionalSummaryForm = ({data,onChange,setResumeData}) => {
    ...
    const generateSummary = async()=>{
        try {
            ...
            setResumeData(prev => ({...prev, professional_summaty: response.data.enhancedContent}))
        } catch (error) {
  ...
  return (  
    <div className='space-y-4'>
        ...
        <div className='mt-6'>
            <textarea value={data || ""} onChange={(e)=>onChange(e.target.value)} rows={7} className='w-full p-3 px-4 mt-2 border text-sm border-gray-300 rounded-lg focus:ring focus:ring-green-500 focus:border-green-500 outline-none transition-colors resize-none' placeholder='Write a Compilling Professional Summary that Highlights Your Key Strength and Career Objectives...'/>
